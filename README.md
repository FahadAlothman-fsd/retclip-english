...
## Problem Statement
Retinal diseases such as diabetic retinopathy and glaucoma are major causes of vision loss. Diagnosing them from fundus images requires expert review, which is slow and subjective. The goal is to build an AI system that uses fundus images and textual information (synthetic prompts from disease labels) to detect multiple retinal diseases automatically, improving accuracy and consistency even with limited labeled data.
